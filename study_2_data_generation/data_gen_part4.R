# this script generates simulated data for conditions that test difficulty range covers both classes (8 conditions)
# including response data, initial values and specifications of IRT models for the use in OpenBUGS

# importing data generation scripts (same utility R script as used in data_gen_part2.R)
source("./util/part2_data_generation.R")
source("./util/part4_models.R")
source("./util/part4_initials.R")
# loading R packages
library(purrr)
library(rlist)
library(dplyr)
...
sim_n <- c(600L, 1200L)
# Set number of items
# suppose all items are dichotomous
item_n <- 50
# set random seeds to generate reproductive results
# seed_vector_xxxx <- sample(1000:9999, 1000)
# write.csv(seed_vector_xxxx, "seed_vector_xxxx_2023Oct26.csv", row.names = F)
seed_vector <- read.csv("seed_vector_xxxx_2023Oct26.csv") %>%
  `$`(x)
N_chain <- 3
...
# class proportions
prop_v <- c("3:1", "5:1")
# since we only generate response data for conditions that test difficulty range covers both classes, the vector below only contains one element
# for conditions that test difficulty range covers both latent classes, see part1 and part2 scripts
# 'full' indicating test difficulty range covers both classes
range_v <- c("full")
...
s <- s + 1
# generate person class indices needed for 8 conditions
...
# item parameters are imported from external files that were generated from Part 3
# diff refers to item difficulty
# disc refers to item discrimination
# c1 refers to Major Class, c2 refers to Minor Class
# m1 refers to Minor Class mean = 1; m25 refers to Minor Class mean = 2.5
diff_c1_m1 <- read.csv('diff_c1_m1_55.csv')[['x']]
diff_c1_m25 <- read.csv('diff_c1_m25_55.csv')[['x']]

diff_c2_m1 <- read.csv('diff_c2_m1_55.csv')[['x']]
diff_c2_m25 <- read.csv('diff_c2_m25_55.csv')[['x']]

disc_c1_m1 <- read.csv('disc_c1_m1_55.csv')[['x']]
disc_c1_m25 <- read.csv('disc_c1_m25_55.csv')[['x']]

disc_c2_m1 <- read.csv('disc_c2_m1_55.csv')[['x']]
disc_c2_m25 <- read.csv('disc_c2_m25_55.csv')[['x']]

# minimum and maximum of item difficulties
diff_c1_m25_max <- diff_c1_m25 %>% range() %>% `[`(2)
diff_c1_m25_min <- diff_c1_m25 %>% range() %>% `[`(1)

diff_c1_m1_max <- diff_c1_m1 %>% range() %>% `[`(2)
diff_c1_m1_min <- diff_c1_m1 %>% range() %>% `[`(1)
  ...
  for(j in 1:nrow(ssppmd)){
    if(md[j] == 'separated'){
      repeat{
        ...
        temp_range <- range(temp_ab)
        # make sure the range of item difficulties covers the both classes
        if(dplyr::between(temp_range[1] - diff_c1_m25_min, left = 0, right = 0.15) & dplyr::between(temp_range[2] - diff_c1_m25_max, left = -0.15, right = 0)){
      ...
      repeat{
        ...
        temp_range <- range(temp_ab)
        # make sure the range of item difficulties covers the both classes
        if(dplyr::between(temp_range[1] - diff_c1_m1_min, left = 0, right = 0.15) & dplyr::between(temp_range[2] - diff_c1_m1_max, left = -0.15, right = 0)){
          ability_list[[j]] <- temp_ab
          break
        }
      }
...
for(j in 1:nrow(df_conditions)){
  ...
  if(df_conditions$M_distance[j] == 'separated'){
    write.csv(diff_c1_m25, 'item_diff_c1_m25.csv', row.names = F)
    write.csv(disc_c1_m25, 'item_disc_c1_m25.csv', row.names = F)
    ...
    write.csv(diff_c1_m1, 'item_diff_c1_m1.csv', row.names = F)
    write.csv(disc_c1_m1, 'item_disc_c1_m1.csv', row.names = F)
...
copy_data <- function(destination = dirname(getwd()), from_folder = NULL, N_chain = 3){
  ...
  for(n in 1:N_chain){
    # new_folder <- paste0(getwd(), '/', current_folder, '_', from_folder, '_chain', n)
...
rep_data_gen <- function(myseed = NULL, N_sim = NULL, N_item = NULL, myprop = NULL, mean_dist = NULL, N_chain = 3){
  ...
  # get item difficulty
  diff_c1_file <- list.files(path = '../', pattern = 'item_diff_c1')
  difficulty_c1 <- read.csv(paste0('../', diff_c1_file)) %>%
    `$`(x)
  diff_c2_file <- list.files(path = '../', pattern = 'item_diff_c2')
  difficulty_c2 <- read.csv(paste0('../', diff_c2_file)) %>%
    `$`(x)
  # get item discrimination
  disc_c1_file <- list.files(path = '../', pattern = 'item_disc_c1')
  discrimination_c1 <- read.csv(paste0('../', disc_c1_file)) %>%
    `$`(x)
...
walk(dirs_target, ~{
  diff_c1_file <- list.files(path = .x, pattern = 'item_diff_c1')
  diff_c1 <- read.csv(paste0(.x, '/', diff_c1_file)) %>%
    `$`(x)
  diff_c2_file <- list.files(path = .x, pattern = 'item_diff_c2')
  diff_c2 <- read.csv(paste0(.x, '/', diff_c2_file)) %>%
    `$`(x)
  ...
  # temp_ab_c2 <- temp_ab[temp_idx == 2]
  # print(c(range(diff_c1), range(diff_c2), range(temp_ab)))
  print(c(range(diff_c1), range(temp_ab)))
