# this script processes results from data_gen_part3.R and obtains item parameters for the Minor Class
...
# results of Minor Class (mean = 1) and (mean = 2.5) are stored in separate subfolders
# note results not provided in this repository
setwd('./M1')
setwd('./M25')
...
# setting working directory back to the parent folder
setwd('../')

# results for Minor Class (mean = 1)
log_m1 <- parse_log(loglist = log_list)
map(log_m1, ~{round(.x[[1]][['b']], 2) %>% sort()})
...
map(log_m25, ~{round(.x[[1]][['b']], 2) %>% sort()})
map(log_m25, ~{round(.x[[2]], 2)})
theta_m25 <- map_dbl(log_list, ~{.x['theta_mean', 'mean']})

##############################################
# importing item parameters for the Major Class
diff_c1_m1 <- read.csv('diff_c1_m1.csv') %>% .$x
disc_c1_m1 <- read.csv('disc_c1_m1.csv') %>% .$x

# item parameters for the Minor Class (mean = 1)
# adding anchoring items to the end of the vector
diff_c1_m1_55 <- c(diff_c1_m1, c(-2, -1, 0, 1, 2))
write.csv(diff_c1_m1_55, file = 'diff_c1_m1_55.csv', row.names = F)
hist(diff_c1_m1, breaks = 15)
range(diff_c1_m1)
mean(diff_c1_m1)

hist(disc_c1_m1, breaks = 15)
range(disc_c1_m1)
mean(disc_c1_m1)
median(disc_c1_m1)
disc_c1_m1_55 <- c(disc_c1_m1, rep(mean(disc_c1_m1), 5))
write.csv(disc_c1_m1_55, file = 'disc_c1_m1_55.csv', row.names = F)

# item parameters for the Minor Class (mean = 2.5)
diff_c1_m25 <- read.csv('diff_c1_m25.csv') %>% .$x
disc_c1_m25 <- read.csv('disc_c1_m25.csv') %>% .$x
# adding anchoring items to the end of the vector
diff_c1_m25_55 <- c(diff_c1_m25, c(-2, -1, 0, 1, 2))
write.csv(diff_c1_m25_55, file = 'diff_c1_m25_55.csv', row.names = F)
hist(diff_c1_m25, breaks = 15)
range(diff_c1_m25)
mean(diff_c1_m25)

hist(disc_c1_m25, breaks = 15)
range(disc_c1_m25)
mean(disc_c1_m25)
median(disc_c1_m25)
disc_c1_m25_55 <- c(disc_c1_m25, rep(mean(disc_c1_m25), 5))
write.csv(disc_c1_m25_55, file = 'disc_c1_m25_55.csv', row.names = F)
...
diff_agg <- cbind(diff_c1_m1, diff_c1_m25) %>% 
  cbind(b_m1_est) %>% 
  cbind(b_m25_est) %>% 
  round(digits = 2) %>% 
  as.data.frame() %>% 
...
range(b_m25_est)
mean(b_m25_est)
cbind(diff_c1_m25, b_m25_est) %>% 
  round(digits = 2) %>% 
  as.data.frame() %>% 
  arrange(diff_c1_m25)

# saving item parameters for Minor Class, including the anchoring items
write.csv(c(a_m1_est, rep(mean(disc_c1_m1), 5)), file = 'disc_c2_m1_55.csv', row.names = F)
write.csv(c(a_m25_est, rep(mean(disc_c1_m25), 5)), file = 'disc_c2_m25_55.csv', row.names = F)
